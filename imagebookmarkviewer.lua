--[[--
ImageBookmarkViewer displays the bookmarked image with some simple manipulation options.
This code is based on ui/widget/imageviewer.lua.
]]
...
local ImageBookmarkViewer = InputContainer:extend {
    image_bookmarks = nil,
    doc_path = nil,
    settings = nil,
    idx = 0,
    ...
    -- Whether to remember pan & zoom and rotation of images
    remember_pan_and_zoom = true,
    remember_rotation = true,
    ...
    -- Start with buttons hidden (long tap on screen will toggle their visibility)
    ...
    _render_callback = nil, -- callback to run after next rendering
...
function ImageBookmarkViewer:renderImages()
    -- Create a table of functions that render images from memory
    local image_loaders = {}
    for i, data in ipairs(self.image_bookmarks.image_datas) do
        table.insert(image_loaders, function()
            if data then
    ...
    self.image = image_loaders
    self._images_list = self.image
    self.images_list_nb = #image_loaders
    self._images_list_nb = #image_loaders

    if self._render_callback then
        self._render_callback()
        self._render_callback = nil
    end
end

function ImageBookmarkViewer:onImagesReloaded()
    self:renderImages()
end

function ImageBookmarkViewer:onToggleButtons()
    self.buttons_visible = not self.buttons_visible
    self:update()
end

function ImageBookmarkViewer:init()
    self:renderImages()

    if Device:hasKeys() then
        local physical_button_map = self.settings:getPageTurnButtonMap()
        if physical_button_map == Const.PAGE_TURN_BUTTON_MAP.ZOOM then
            self.key_events = {
                Close = { { Device.input.group.Back } },
                ZoomIn = { { Device.input.group.PgBack } },
                ZoomOut = { { Device.input.group.PgFwd } },
            }
        elseif physical_button_map == Const.PAGE_TURN_BUTTON_MAP.CLOSE_MENU then
            self.key_events = {
                Close = { { Device.input.group.Back }, { Device.input.group.PgFwd } },
                ToggleButtons = { { Device.input.group.PgBack } },
            }
        else -- physical_button_map == Const.PHYSICAL_BUTTON_MAP.CYCLE
            self.key_events = {
                Close = { { Device.input.group.Back } },
    ...
    -- if self.image is a list of images, swap it with image of index self.idx to be displayed
    if type(self.image) == "table" then
        self._images_list = self.image
        self.idx = self.settings:getIdx(self.doc_path)
        self.image = self._images_list[self.idx]
    ...
    if self.remember_rotation then
        self.rotation = self.settings:getBookmarks(self.doc_path)[self.idx].rotation or 0
    elseif self.image and G_reader_settings:isTrue("imageviewer_rotate_auto_for_best_fit") then
        if (Screen:getWidth() > Screen:getHeight()) ~= (self.image:getWidth() > self.image:getHeight()) then
            self.rotation = 1
        end
    end
        ...
        {
            {
                ...
                text = "Move left",
                callback = function()
                    local idx_to = self.idx - 1
                    if idx_to == 0 then
                        idx_to = #self.settings:getBookmarks(self.doc_path)
                    end
                    self._render_callback = function()
                        self:switchToImageNum(idx_to)
                    end
                    local success = self.settings:moveBookmark(self.doc_path, self.idx, idx_to)
                    if not success then
                        self._render_callback = nil
                    end
                end,
            ...
            {
                id = "scale",
                text = self.scale_factor == 0 and _("Original size") or _("Scale"),
                callback = function()
                    self.scale_factor = self.scale_factor == 0 and 1 or 0
            ...
            },
            {
                ...
                callback = function()
                    UIManager:show(ConfirmBox:new {
                        text = "Remove image bookmark?",
                        ok_callback = function()
                            self._render_callback = function()
                                -- No images remaining -> close
                                if self.images_list_nb < 1 then
                                    self:onClose()
                                    -- At least one image remains -> move to previous image
                                else
                                    local new_idx = math.max(self.idx - 1, 1)
                                    self:switchToImageNum(new_idx)
                                end
                            end
            ...
            },
            {
                ...
                callback = function()
                    local idx_to = self.idx + 1
                    if idx_to > #self.settings:getBookmarks(self.doc_path) then
                        idx_to = 1
                    end
                    self._render_callback = function()
                        self:switchToImageNum(idx_to)
    ...
    if self._images_list and self._images_list_nb > 1 and self.progress_bar_enabled then
        -- progress bar
        local percent = (self.idx - 1) / (self._images_list_nb - 1)
    ...
    self._center_x_ratio = self.settings:getBookmarks(self.doc_path)[self.idx].center_x_ratio
    self._center_y_ratio = self.settings:getBookmarks(self.doc_path)[self.idx].center_y_ratio
    self.scale_factor = self.settings:getBookmarks(self.doc_path)[self.idx].scale_factor
    self:update()
end

function ImageBookmarkViewer:update()
    ...
    if self.buttons_visible then
        ...
        scale_btn:setText(self.scale_factor == 0 and _("Original size") or _("Scale"), scale_btn.width)
    ...
    self.settings:updateBookmark(self.doc_path, self.idx, true, self._center_x_ratio, self._center_y_ratio,
        self.scale_factor, self.rotation)
end

function ImageBookmarkViewer:_clean_image_wg()
...
function ImageBookmarkViewer:_new_image_wg()
    ...
    local rotation_angle = 360 - self.rotation * 90
...
function ImageBookmarkViewer:onShow()
    self.dithered = true
    UIManager:setDirty(self, function()
...
function ImageBookmarkViewer:switchToImageNum(image_num)
    -- if image_num == self.doc_settings.idx then
    --     return
    -- end
    ...
    self._center_y_ratio = self.settings:getBookmarks(self.doc_path)[self.idx].center_y_ratio
    self.scale_factor = self.settings:getBookmarks(self.doc_path)[self.idx].scale_factor
    self.rotation = self.settings:getBookmarks(self.doc_path)[self.idx].rotation
    self:update()
end

-- Image switching events
function ImageBookmarkViewer:onShowNextImage()
    if self.idx < self._images_list_nb then
        self:switchToImageNum(self.idx + 1)
    else
        self:switchToImageNum(1)
    end
end

function ImageBookmarkViewer:onShowPrevImage()
    if self.idx > 1 then
        self:switchToImageNum(self.idx - 1)
    else
        self:switchToImageNum(self._images_list_nb)
    end
end

function ImageBookmarkViewer:onTap(_, ges)
...
function ImageBookmarkViewer:panBy(x, y)
    if self._image_wg then
...
-- Panning events
function ImageBookmarkViewer:onSwipe(_, ges)
...
function ImageBookmarkViewer:onMultiSwipe(_, ges)
...
function ImageBookmarkViewer:onHold(_, ges)
    -- Start of pan
    self._panning = true
...
function ImageBookmarkViewer:onHoldRelease(_, ges)
    -- End of pan
...
function ImageBookmarkViewer:onPan(_, ges)
    self._panning = true
...
function ImageBookmarkViewer:onPanRelease(_, ges)
    if self._panning then
        self._panning = false
        self:panBy(-self._pan_relative_x, -self._pan_relative_y)
    end
...
-- Zoom events
function ImageBookmarkViewer:_refreshScaleFactor()
    if self.scale_factor == 0 then
...
function ImageBookmarkViewer:_applyNewScaleFactor(new_factor)
...
function ImageBookmarkViewer:onZoomIn(inc)
    self:_refreshScaleFactor()
...
function ImageBookmarkViewer:onZoomOut(dec)
    self:_refreshScaleFactor()
...
function ImageBookmarkViewer:onSpread(_, ges)
    ...
    self._center_x_ratio, self._center_y_ratio = self._image_wg:getPanByCenterRatio(ges.pos.x - Screen:getWidth() / 2,
...
function ImageBookmarkViewer:onPinch(_, ges)
    if not self._image_wg then
        return
    end
...
function ImageBookmarkViewer:onTapDiagonal()
    return self:onSaveImageView()
end

function ImageBookmarkViewer:onSaveImageView()
...
function ImageBookmarkViewer:onClose()
    -- UIManager:broadcastEvent(Event:new("DocSettingsUpdated", self.doc_settings))
    self.settings:flush()
    UIManager:close(self)
    return true
end

function ImageBookmarkViewer:onCloseWidget()
...
function ImageBookmarkViewer:register(registry)
    registry:addAuxProvider({
        ...
        callback = ImageBookmarkViewer.openFile,
        disable_file = true,
        disable_type = false,
    })
end

function ImageBookmarkViewer.openFile(file)
    UIManager:show(ImageBookmarkViewer:new {
        file = file,
        fullscreen = true,
